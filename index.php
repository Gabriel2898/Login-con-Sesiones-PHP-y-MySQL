...
if(isset($_POST["ingresar"])){
  $ussuario = mysqli_real_escape_string($conn, $_POST['user']);
  $passsword = mysqli_real_escape_string($conn, $_POST['pass']);
  $password_encryptt = sha1($passsword);
$sqll="SELECT idusuarios FROM usuarios WHERE  usuario='$ussuario' AND password='$password_encryptt'";
$resultadoo=$conn->query($sqll);
$rowss=$resultadoo->num_rows;
if($rowss>0){
  $roow=$resultadoo->fetch_assoc();
  $_SESSION['id_usuario']=$roow['idusuarios'];
  header("Location: admin.php");
}
echo "<script>alert(' El usuario o la contraseña esta incorrecto' );
 ...
 if(isset($_POST["registrar"])){
   ...
   if($filas>0){
     ...
     $sqlusuario="INSERT INTO usuarios (Nombre,Correo,usuario, password) VALUES ('$nombre','$correo','$usuario','$password_encrypt')";
